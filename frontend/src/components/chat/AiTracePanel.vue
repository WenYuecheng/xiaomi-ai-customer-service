...
const stageLabels: Record<AiTraceStep['stage'], string> = {
  understanding: 'DeepSeek 调用 1/3 · 理解问题',
  retrieval: 'BGE · 召回候选知识',
  reranking: 'DeepSeek 调用 2/3 · AI 重排',
  generation: 'DeepSeek 调用 3/3 · 生成回答',
...
<template>
  <details class="ai-trace" open>
    <summary>
      ...
      <span class="ai-trace__count">{{ steps.length }}/5 阶段</span>
    </summary>
    <ol class="ai-trace__steps">
      <li v-for="step in orderedSteps" :key="step.stage" :class="[`is-${step.status}`, `is-${step.stage}`]">
        ...
        <div class="ai-trace__main">
          ...
          <p>{{ step.summary }}</p>
          <ul v-if="step.details?.length" class="ai-trace__details">
...
.ai-trace__details { display: grid; gap: 4px; list-style: none; margin: 6px 0 7px; padding: 0; }.ai-trace__details li { background: rgba(255,255,255,.76); border: 1px solid rgba(124,98,221,.14); border-radius: 8px; color: #615475; font-size: 10px; line-height: 1.45; padding: 5px 7px; }.ai-trace__details li::before { color: #765cce; content: '\2713'; font-weight: 700; margin-right: 5px; }
