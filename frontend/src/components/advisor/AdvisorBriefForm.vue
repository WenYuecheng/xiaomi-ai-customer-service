...
const form = reactive({
  knowledge_base_ids: [] as string[], category: 'phone' as Category, mode: 'purchase_advice' as const,
...
function submit(): void {
  if (!form.knowledge_base_ids.length || !form.message.trim()) return
  emit('submit', {
...
<template>
  <form class="brief-console" @submit.prevent="submit">
    ...
    <div class="field-grid">
      <label class="knowledge-field"><span>知识库范围（最多 5 个）</span><KnowledgeBaseMultiSelect v-model="form.knowledge_base_ids" :items="knowledgeBases" storage-key="xmcs_advisor_knowledge_base_ids" /></label>
    ...
    <button class="launch-button" type="submit" :disabled="busy || !form.knowledge_base_ids.length"><span>{{ busy ? '正在检索证据并生成方案…' : '生成有依据的选购方案' }}</span><i>→</i></button>
  </form>
</template>
