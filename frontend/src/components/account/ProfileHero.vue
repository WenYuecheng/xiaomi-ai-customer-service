...
<template>
  <section class="profile-hero">
    ...
    <header><span>PERSONAL AI SPACE / {{ user.avatar_key.toUpperCase() }}</span><b>账户安全 ●</b></header>
    <div class="profile-hero__main">
      <div class="avatar" :data-avatar="user.avatar_key">{{ user.display_name.slice(0, 1).toUpperCase() }}</div>
      <div><p>{{ greeting }}，</p><h1>{{ user.display_name }}</h1><div class="identity"><span>{{ roleLabels[user.role] }}</span><span>已加入 {{ dashboard.joined_days }} 天</span><span>AI 探索者 Lv.{{ dashboard.growth_level }}</span></div></div>
    </div>
    <div class="profile-hero__actions"><button type="button" @click="emit('edit')">编辑数字身份</button><button type="button" @click="emit('password')">账户安全</button></div>
