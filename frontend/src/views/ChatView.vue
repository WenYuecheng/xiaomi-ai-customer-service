...
<template>
  <section class="chat-page">
    <header class="page-heading"><div><span>AI CUSTOMER SERVICE</span><h1>你好，需要什么帮助？</h1><p>产品参数、使用指导、选购对比与售后政策，都可以直接问我。</p></div><div class="chat-controls"><el-select v-model="knowledgeBaseId" aria-label="当前知识库" placeholder="选择知识库" @change="changeKnowledgeBase"><el-option v-for="item in knowledgeBases" :key="item.id" :label="item.name" :value="item.id" /></el-select><el-button @click="clearConversation">新建对话</el-button></div></header>
