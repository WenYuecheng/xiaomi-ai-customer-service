...
function renderGraph(): void {
  ...
  const categories = ['知识库', '品类', '产品', '文档'].map((name) => ({ name }))
...
<template>
  <section v-loading="loading" class="knowledge-analytics">
    ...
    <div class="visual-grid">
      <div class="graph-wrap"><div ref="chart" data-testid="knowledge-graph" class="graph-canvas" role="img" aria-label="知识库、品类、产品与文档关系图" /><div v-if="!graph.nodes.length && !loading" class="graph-empty"><b>暂无可视化节点</b><span>上传并完成处理后，关系图会自动生成</span></div></div>
...
<style scoped>
.knowledge-analytics{background:#fff;border:1px solid var(--line);border-radius:20px;box-shadow:var(--shadow-sm);overflow:hidden;padding:24px}.analytics-heading{align-items:flex-end;display:flex;justify-content:space-between}.analytics-heading>div>span{color:var(--mi-orange);font-family:var(--font-mono);font-size:10px;font-weight:700;letter-spacing:.14em}.analytics-heading h2{font-size:25px;margin:6px 0}.analytics-heading p{color:var(--ink-muted);margin:0}.legend{color:var(--ink-muted);font-size:11px}.legend i{background:var(--mi-orange);border-radius:50%;display:inline-block;height:7px;margin-right:6px;width:7px}.stat-grid{display:grid;gap:10px;grid-template-columns:repeat(4,1fr);margin:20px 0 12px}.stat-grid article{align-items:center;background:#f8f7f5;border:1px solid var(--line);border-radius:14px;display:flex;gap:12px;padding:15px}.stat-grid svg{color:var(--mi-orange);height:23px;width:23px}.stat-grid strong{display:block;font-size:24px;font-variant-numeric:tabular-nums}.stat-grid span{color:var(--ink-muted);font-size:11px}.visual-grid{display:grid;gap:12px;grid-template-columns:minmax(0,1fr) 220px}.graph-wrap{position:relative}.graph-canvas{background:#fbfaf8;border:1px solid var(--line);border-radius:16px;height:390px;min-width:0}.graph-empty{display:grid;inset:0;place-content:center;pointer-events:none;position:absolute;text-align:center}.graph-empty b,.graph-empty span{display:block}.graph-empty span{color:var(--ink-muted);font-size:11px;margin-top:5px}.category-panel{background:#1d1c1a;border-radius:16px;color:#fff;padding:16px}.category-panel h3{font-size:14px;margin:0 0 14px}.category-panel>div{border-bottom:1px solid #34322f;display:flex;justify-content:space-between;padding:10px 0}.category-panel span{font-size:12px}.category-panel b{color:#f6a069;font-size:10px}
...
<style scoped>
.category-panel{background:#faf9f7;border:1px solid var(--line);color:var(--ink)}.category-panel>div{border-bottom-color:var(--line)}.category-panel b{color:var(--mi-orange)}
</style>
