...
| EP02-02 多轮上下文 | 会话、摘要、问题改写 | 追问与隔离测试 |
| EP02-03 来源引用 | `MessageSource`、来源卡片 | grounded source 测试 |
| EP02-04 低置信兜底 | 阈值与 `FALLBACK_ANSWER` | 无关问题、连续兜底测试 |
| EP02-05 检索参数 | 环境配置 `TOP_K`/阈值 | Pydantic 范围校验 |
| EP02-06 JSON/SSE | JSON 与五类 SSE 事件 | SSE 顺序测试 |
| EP03-01 聊天主页 | `ChatView.vue` | 前端构建 |
| EP03-02 流式与停止 | `api/chat.ts`、AbortController | SSE 测试、前端构建 |
...
| EP06-02 核心测试 | pytest/Vitest | 后端 23、前端 2 |
| EP06-03 Compose MVP | 两个 Dockerfile、Compose | 配置静态验证；守护进程待启动 |
